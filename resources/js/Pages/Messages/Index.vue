...
import {
    ...
    PaperClipIcon, EllipsisVerticalIcon, ArrowLeftIcon,
...
<template>
    ...
    <SmartHealthLayout title="" :full-bleed="true">
        <div class="flex h-[calc(100vh-4rem)] overflow-hidden">
            ...
            <!-- Mobile: visible only when no thread is active -->
            ...
            <div
                ...
                :class="active_thread ? 'hidden lg:flex lg:w-80 lg:shrink-0' : 'flex w-full lg:w-80 lg:shrink-0'"
            >
                <!-- Panel header -->
            ...
            <!-- ── RIGHT PANEL (chat) ── -->
            <!-- Mobile: visible only when a thread is active -->
            <!-- Desktop (lg+): always visible, fills remaining width -->
            <div
                class="flex flex-col min-w-0 bg-white dark:bg-slate-950"
                :class="active_thread ? 'flex flex-1' : 'hidden lg:flex lg:flex-1'"
            >
                <!-- Empty state (desktop only when no thread) -->
                ...
                <template v-else>
                    <!-- Conversation header -->
                    <div class="flex items-center justify-between px-4 sm:px-6 py-3.5 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                        <div class="flex items-center gap-2 sm:gap-3 min-w-0">
                            <!-- Back button — mobile only -->
                            <Link :href="route('messages.index')"
                                class="lg:hidden grid h-8 w-8 shrink-0 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                                <ArrowLeftIcon class="h-5 w-5" />
                            </Link>
                            <span class="relative grid h-9 w-9 sm:h-10 sm:w-10 shrink-0 place-items-center rounded-full text-white text-sm font-semibold"
                        ...
                        <div class="flex items-center gap-1 shrink-0">
                            <button class="grid h-9 w-9 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                                <PhoneIcon class="h-[18px] w-[18px]" />
                            </button>
                            <button class="hidden sm:grid h-9 w-9 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                    ...
                    <!-- Messages area -->
                    <div ref="messagesEl" class="flex-1 overflow-y-auto px-3 sm:px-6 py-4 space-y-1 scrollbar-thin">
                        <template v-for="item in groupedMessages" :key="item.id ?? item.label">
                            ...
                            <div v-else
                                ...
                                <!-- Avatar -->
                                <span class="grid h-7 w-7 sm:h-8 sm:w-8 shrink-0 place-items-center rounded-full text-white text-xs font-semibold mb-1"
                                    :class="avatarColor(item.sender_name)">
                                    {{ item.sender_initials }}
                                </span>

                                <!-- Bubble + meta -->
                                <div class="max-w-[72vw] sm:max-w-sm lg:max-w-md" :class="item.sender_id === me?.id ? 'items-end' : 'items-start'" style="display:flex;flex-direction:column;">
                                    ...
                                    <div class="flex items-end gap-2"
                                        :class="item.sender_id === me?.id ? 'flex-row-reverse' : 'flex-row'">
                                        <!-- Bubble -->
                                        <div class="relative px-3 sm:px-4 py-2 sm:py-2.5 rounded-2xl text-sm leading-relaxed"
                    ...
                    <!-- Reply input -->
                    <div class="border-t border-slate-200 dark:border-slate-800 px-3 sm:px-4 py-3 bg-white dark:bg-slate-900">
                        <form @submit.prevent="sendReply" class="flex items-center gap-2">
                            <button type="button" class="hidden sm:grid h-9 w-9 shrink-0 place-items-center rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
